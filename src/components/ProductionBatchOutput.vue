<template>
    <div>
      <h4 class="subtitle-1 pt-8">Output</h4>
      <v-card
        ...
        <div
          ...
          <v-card class="pa-4 white mt-2">
            <v-row class="d-flex align-start justify-center">
              ...
              <v-col cols="12" md="2" class="d-flex flex-column justify-space-between">
                <div class="overline align-self-md-start pl-0" >Good Qty</div>
                <div class="body-1 align-self-md-start" >{{product.goodQuantity}}</div>
              </v-col>
              <v-col cols="12" md="2" class="d-flex flex-column justify-space-between">
...
function edit (batchDataView, state, e, product) {
  ...
  state.output.goodQuantity = Number(product.goodQuantity)
  ...
  const saveData = (batchDataView, state) => {
    ...
    if (!saveProductNames.includes(state.output.productName)) {
      Object.assign(newProduct, {
        id: Number(state.products.find((product) => product.name === state.output.productName).id),
        name: state.output.productName,
        goodQuantity: Number(state.output.goodQuantity),
        damagedQuantity: state.output.damagedQuantity,
        price: Number(state.products.find((product) => product.name === state.output.productName).price.price)
